    ...
    def get(self, request):
        ...
        if Permissions.is_manager(request):
            ...
            for order in orders:
                ...
                serialized_orders.append(serialized_order)
            return Response(serialized_orders)
        if Permissions.is_delivery_crew(request):
            orders = Order.objects.filter(delivery_crew = request.user)
            serialized_orders =[]
            for order in orders:
                serialized_order = OrderSerializer (order).data 
                order_items= OrderItem.objects.filter(order=order)
                serialized_order_item= OrderItemSerializer(order_items,many=True).data
                serialized_order['items']=serialized_order_item
                serialized_orders.append(serialized_order)
            return Response(serialized_orders)
        return Response(status=status.HTTP_403_FORBIDDEN)